...
if (turbineCanvas) {
    ...
    // Adjust camera distance based on screen size so transformer fits on mobile
    const getHeroCameraZ = () => window.innerWidth < 768 ? 38 : (window.innerWidth < 1024 ? 30 : 22);
    camera.position.set(5, 5, getHeroCameraZ());
...
if (plexusContainer) {
    ...
    // Adjust camera distance based on screen size so globe fits on mobile
    const getCameraZ = () => window.innerWidth < 768 ? 330 : (window.innerWidth < 1024 ? 380 : 280);
